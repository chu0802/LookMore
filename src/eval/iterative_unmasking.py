...
class IterativeUnmaskTester(Tester):
    def __init__(self, *args, iterative_token_predictor=None, gt_attn_predictor=None, iteration=2, k_ratio=0.1, initial_mask_ratio=0.1, **kwargs):
        super().__init__(*args, **kwargs)
        self.iterative_token_predictor = iterative_token_predictor
        self.gt_attn_predictor = gt_attn_predictor
    ...
    def dump_residual_maps(self, threshold=10):
    ...
    def test(self):
        for i, batch in tqdm(enumerate(self.dataloader), total=len(self.dataloader)):
            ...
            # build initial masks
            original_selector_dict = self.gt_attn_predictor(images)
...
def main(args):
    ...
    lw = load_model(
        ...
        selector_params_path=args.teacher_selector_params_path,
        mask_prediction=True,
    )

    iterative_token_predictor = load_model(
        pretrained_params_path=args.pretrained_params_path, 
        head_params_path=args.head_params_path, 
    ...
    gt_attn_predictor = load_model(
        pretrained_params_path=args.pretrained_params_path, 
        head_params_path=args.head_params_path, 
    ).selector

    lw.eval()
    iterative_token_predictor.eval()
    gt_attn_predictor.eval()
    ...
    tester = IterativeUnmaskTester(lw, iterative_token_predictor=iterative_token_predictor, gt_attn_predictor=gt_attn_predictor, dataloader=dataloader, output_dir=args.output_dir, iteration=args.iteration)
    tester.test()

if __name__ == "__main__":
    args = argument_parser(
        Arg("--pretrained_params_path", type=str, default="models/lookwhere_dinov2.pt"),
        Arg("--selector_params_path", type=str, default="/home/yuchuyu/project/lookwhere/iterative_mae_res_prediction/models/seed_1102_epoches_10_pretrained/selector_epoch_10.pt"),
        Arg("--teacher_selector_params_path", type=str, default="/home/yuchuyu/project/lookwhere/mae_head/models/seed_1102_epoches_10_pretrained/selector_epoch_10.pt"),
        ...
        Arg("--output_dir", type=Path, default=Path("/home/yuchuyu/project/lookwhere/output/iterative_mae_res_prediction_unmask_physically_token_dropping")),
